...
function result = process_load_to_structured_data(txtFileName, profileNumber)
% PROCESS_LOAD_TO_STRUCTURED_DATA
%
% Egy megadott ElectricityLoadDiagrams20112014 fogyasztasi adatsort dolgoz fel
% PV+BESS szimulaciohoz.
%
% Hasznalat:
%
%   result = process_load_to_structured_data('LD2011_2014.txt', 86);
%   result = process_load_to_structured_data('LD2011_2014.txt', "MT_086");
%
% Automatikus mappaszerkezet:
%
%   current_function_folder/
...
%
% Fontos:
%   - Nincs automatikus profilvalasztas.
%   - Nincs ipari/commercial/residential szures.
%   - A megadott fogyasztoi adatsort dolgozza fel.
%   - Az eredeti mintavetelezest tartja meg, tehat 15 perces inputnal
%     15 perces napi fajlokat ment.

    % ---------------------------------------------------------------------
    % 0) Bemenetek es automatikus utvonalak
    % ---------------------------------------------------------------------
    if nargin < 1 || strlength(string(txtFileName)) == 0
        error('Meg kell adni a bemeneti txt fajl nevet. Pelda: process_load_to_structured_data(''LD2011_2014.txt'', 86)');
    end

    if nargin < 2 || isempty(profileNumber)
        error('Meg kell adni a feldolgozando fogyaszto sorszamat vagy nevet. Pelda: 86 vagy "MT_086".');
    end
    ...
    if ~isfile(txtFilePath)
        error('A megadott txt fajl nem talalhato: %s', txtFilePath);
    end
    ...
    P_all_kW = table2array(T(:, 2:end));
    P_all_kW(P_all_kW < 0) = NaN;

    nSamples = size(P_all_kW, 1);
    nCustomers = size(P_all_kW, 2);

    fprintf('Mintak szama: %d\n', nSamples);
    fprintf('Fogyasztok szama: %d\n', nCustomers);

    % ---------------------------------------------------------------------
    % 4) Kivalasztott fogyaszto azonosítása
    % ---------------------------------------------------------------------
    ...
    selectedPower_kW_raw = P_all_kW(:, selectedIdx);

    activeIdx = selectedPower_kW_raw > 0 & ~isnan(selectedPower_kW_raw);

    if nnz(activeIdx) < 10
        error('A kivalasztott fogyasztasi adatsor nem tartalmaz eleg ervenyes mintat: %s', selectedCustomer);
    end

    % ---------------------------------------------------------------------
    % 5) Mintavetelezes meghatarozasa
    % ---------------------------------------------------------------------
    dtOriginal_h = hours(median(diff(time)));
    ...
    samplesPerDayOriginal = round(24 / dtOriginal_h);
    outputResolutionMin = round(dtOriginal_h * 60);

    fprintf('Kimeneti felbontas: %d perc\n', outputResolutionMin);
    fprintf('Mintak szama egy napban: %d\n', samplesPerDayOriginal);

    % ---------------------------------------------------------------------
    % 6) Aktiv idoszak levagasa
    % ---------------------------------------------------------------------
    firstIdx = find(activeIdx, 1, 'first');
    lastIdx  = find(activeIdx, 1, 'last');

    selectedPower_kW = selectedPower_kW_raw(firstIdx:lastIdx);
    selectedTime = time(firstIdx:lastIdx);

    % ---------------------------------------------------------------------
    % 7) Teljes napi matrix epitese
    % ---------------------------------------------------------------------
    [dayMatrix_kW, dayDates, dailyEnergy_kWh, dailyPeak_kW] = ...
        local_build_complete_day_matrix( ...
            selectedTime, ...
            selectedPower_kW, ...
            samplesPerDayOriginal, ...
            dtOriginal_h);
    ...
    if isempty(dayMatrix_kW)
        error('Nem sikerult teljes napi profilokat epiteni a kivalasztott fogyasztobol.');
    end

    nValidDays = size(dayMatrix_kW, 1);

    fprintf('Teljes, mentheto napok szama: %d\n', nValidDays);
    ...
    totalEnergy_kWh = sum(selectedPower_kW, 'omitnan') * dtOriginal_h;
    activeDays = days(selectedTime(end) - selectedTime(1)) + dtOriginal_h / 24;
    avgDailyEnergy_kWh = totalEnergy_kWh / activeDays;

    averageDailyProfile_kW = mean(dayMatrix_kW, 1, 'omitnan');

    timeOfDay_h = (0:samplesPerDayOriginal-1) * dtOriginal_h;
    timeOfDayDuration = duration(0, 0, 0) + minutes(timeOfDay_h * 60);

    pvWindowMask = timeOfDay_h >= pvPeakStartHour & timeOfDay_h < pvPeakEndHour;

    pvWindowMean_kW = mean(averageDailyProfile_kW(pvWindowMask), 'omitnan');
    pvWindowPeak_kW = max(averageDailyProfile_kW(pvWindowMask), [], 'omitnan');
    pvWindowEnergy_kWh = sum(averageDailyProfile_kW(pvWindowMask), 'omitnan') * dtOriginal_h;
    totalDailyProfileEnergy_kWh = sum(averageDailyProfile_kW, 'omitnan') * dtOriginal_h;
    pvWindowToTotalEnergyRatio = local_safe_divide(pvWindowEnergy_kWh, totalDailyProfileEnergy_kWh);

    fprintf('Atlagos teljesitmeny: %.2f kW\n', meanPower_kW);
    fprintf('Csucsteljesitmeny: %.2f kW\n', peakPower_kW);
    fprintf('Atlagos napi energia: %.2f kWh/nap\n', avgDailyEnergy_kWh);
    fprintf('10-14h atlagteljesitmeny: %.2f kW\n', pvWindowMean_kW);
    fprintf('10-14h csucsteljesitmeny: %.2f kW\n', pvWindowPeak_kW);
    fprintf('10-14h energia aranya: %.2f %%\n\n', pvWindowToTotalEnergyRatio * 100);

    % ---------------------------------------------------------------------
    % 9) Atlagos napi profil plottolasa
    % ---------------------------------------------------------------------
    figAverage = figure('Color', 'w', 'Name', 'Selected load profile');
    hold on;
    grid on;
    box on;

    yMax = max(averageDailyProfile_kW, [], 'omitnan') * 1.15;
    ...
    patch( ...
        [pvPeakStartHour, pvPeakEndHour, pvPeakEndHour, pvPeakStartHour], ...
        [0, 0, yMax, yMax], ...
        [0.90, 0.90, 0.90], ...
        'EdgeColor', 'none', ...
        'FaceAlpha', 0.35, ...
    ...
    plot(timeOfDay_h, averageDailyProfile_kW, ...
        'LineWidth', 2.5, ...
        'DisplayName', sprintf('%s atlagos napi profil', selectedCustomer));

    xlabel('Napon beluli ido [h]');
    ylabel('Atlagos teljesitmeny [kW]');
    title(sprintf('Kivalasztott fogyaszto: %s', selectedCustomer), ...
        'Interpreter', 'none');
    ...
    saveas(figAverage, fullfile(savePath, 'selected_load_profile_average.png'));
    savefig(figAverage, fullfile(savePath, 'selected_load_profile_average.fig'));

    % ---------------------------------------------------------------------
    % 10) Reszletes havi/napi profilabrak
    % ---------------------------------------------------------------------
    figDetailed = local_plot_detailed_profile_figure( ...
        dayMatrix_kW, ...
        dayDates, ...
        dtOriginal_h, ...
        selectedCustomer, ...
    ...
    % ---------------------------------------------------------------------
    % 11) Napi .mat fajlok mentese eredeti felbontasban
    % ---------------------------------------------------------------------
    fprintf('Napi .mat fajlok mentese...\n');

    timeMinAxis = (0:samplesPerDayOriginal-1) * dtOriginal_h * 60;
    ...
    for d = 1:nValidDays
        ...
        currentDay = dayDates(d);

        pDay_kW = dayMatrix_kW(d, :);
        eDay_kWh = pDay_kW * dtOriginal_h;

        if numel(pDay_kW) ~= samplesPerDayOriginal
            skippedDays = skippedDays + 1;
            continue;
        end

        if any(isnan(pDay_kW)) || sum(pDay_kW, 'omitnan') <= 0
            skippedDays = skippedDays + 1;
            continue;
        end
        ...
        consumptionCache.projectName = 'ElectricityLoadDiagrams_Selected_Load_Profile';
        consumptionCache.sourceName = 'ElectricityLoadDiagrams20112014';

        consumptionCache.selectedCustomer = selectedCustomer;
        consumptionCache.selectedColumnIndex = selectedIdx;

        consumptionCache.dateString = datestr(currentDay, 'yyyy.mm.dd');
        consumptionCache.date = currentDay;

        consumptionCache.dt_h = dtOriginal_h;
        consumptionCache.timeMinAxis = timeMinAxis;

        % Korabbi keretrendszerrel kompatibilis mezok
        consumptionCache.powerTotal_W = loadPower_W;
        consumptionCache.energyTotal_Wh = loadEnergy_Wh;
        ...
        % Profiljellemzok
        consumptionCache.averageProfile_meanPower_kW = meanPower_kW;
        consumptionCache.averageProfile_peakPower_kW = peakPower_kW;
        consumptionCache.averageDailyEnergy_kWh = avgDailyEnergy_kWh;

        consumptionCache.pvPeakStartHour = pvPeakStartHour;
        consumptionCache.pvPeakEndHour = pvPeakEndHour;
        consumptionCache.averageDailyPvPeakWindowMean_kW = pvWindowMean_kW;
        consumptionCache.averageDailyPvPeakWindowPeak_kW = pvWindowPeak_kW;
        consumptionCache.averageDailyPvPeakWindowEnergy_kWh = pvWindowEnergy_kWh;
        consumptionCache.averageDailyTotalProfileEnergy_kWh = totalDailyProfileEnergy_kWh;
        consumptionCache.averageDailyPvWindowToTotalEnergyRatio = pvWindowToTotalEnergyRatio;
    ...
    processingSummary.selectedCustomer = selectedCustomer;
    processingSummary.selectedColumnIndex = selectedIdx;
    processingSummary.requestedProfileNumber = profileNumber;

    processingSummary.dtOriginal_h = dtOriginal_h;
    processingSummary.outputResolutionMin = outputResolutionMin;
    processingSummary.samplesPerDay = samplesPerDayOriginal;
    processingSummary.timeMinAxis = timeMinAxis;

    processingSummary.totalSavedDays = totalSavedDays;
    processingSummary.skippedDays = skippedDays;

    processingSummary.meanPower_kW = meanPower_kW;
    processingSummary.peakPower_kW = peakPower_kW;
    processingSummary.totalEnergy_kWh = totalEnergy_kWh;
    processingSummary.avgDailyEnergy_kWh = avgDailyEnergy_kWh;

    processingSummary.pvPeakStartHour = pvPeakStartHour;
    processingSummary.pvPeakEndHour = pvPeakEndHour;
    processingSummary.pvWindowMean_kW = pvWindowMean_kW;
    processingSummary.pvWindowPeak_kW = pvWindowPeak_kW;
    processingSummary.pvWindowEnergy_kWh = pvWindowEnergy_kWh;
    processingSummary.totalDailyProfileEnergy_kWh = totalDailyProfileEnergy_kWh;
    processingSummary.pvWindowToTotalEnergyRatio = pvWindowToTotalEnergyRatio;

    processingSummary.dayMatrix_kW = dayMatrix_kW;
    processingSummary.dayDates = dayDates;
    processingSummary.dailyEnergy_kWh = dailyEnergy_kWh;
    processingSummary.dailyPeak_kW = dailyPeak_kW;
    processingSummary.averageDailyProfile_kW = averageDailyProfile_kW;
    processingSummary.timeOfDay_h = timeOfDay_h;
    processingSummary.timeOfDayDuration = timeOfDayDuration;

    save(fullfile(savePath, 'selected_load_profile_processing_summary.mat'), ...
        'processingSummary');

    % ---------------------------------------------------------------------
    % 13) Kimeneti result struktura
    % ---------------------------------------------------------------------
    result = struct();
    ...
    result.selectedCustomer = selectedCustomer;
    result.selectedColumnIndex = selectedIdx;
    result.requestedProfileNumber = profileNumber;

    result.time = selectedTime;
    result.power_kW = selectedPower_kW;

    result.dt_h = dtOriginal_h;
    result.outputResolutionMin = outputResolutionMin;
    result.samplesPerDay = samplesPerDayOriginal;

    result.dayMatrix_kW = dayMatrix_kW;
    result.dayDates = dayDates;
    result.dailyEnergy_kWh = dailyEnergy_kWh;
    result.dailyPeak_kW = dailyPeak_kW;

    result.averageDailyProfile_kW = averageDailyProfile_kW;
    result.timeOfDay_h = timeOfDay_h;
    result.timeOfDayDuration = timeOfDayDuration;

    result.meanPower_kW = meanPower_kW;
    result.peakPower_kW = peakPower_kW;
    result.totalEnergy_kWh = totalEnergy_kWh;
    result.avgDailyEnergy_kWh = avgDailyEnergy_kWh;

    result.pvPeakStartHour = pvPeakStartHour;
    result.pvPeakEndHour = pvPeakEndHour;
    result.pvWindowMean_kW = pvWindowMean_kW;
    result.pvWindowPeak_kW = pvWindowPeak_kW;
    result.pvWindowEnergy_kWh = pvWindowEnergy_kWh;
    result.totalDailyProfileEnergy_kWh = totalDailyProfileEnergy_kWh;
    result.pvWindowToTotalEnergyRatio = pvWindowToTotalEnergyRatio;

    result.figures = struct();
    result.figures.averageProfile = figAverage;
    result.figures.detailedProfile = figDetailed;
...
function selectedIdx = local_resolve_profile_index(profileNumber, customerNames)
    ...
    if isnumeric(profileNumber)
        ...
        if ~isscalar(profileNumber) || isnan(profileNumber)
            error('A profileNumber numerikus esetben egyetlen skalar ertek legyen.');
        end

        profileNumber = round(profileNumber);

        candidateName = string(sprintf('MT_%03d', profileNumber));

        selectedIdx = find(upper(customerNames) == upper(candidateName), 1, 'first');
        ...
        if profileNumber >= 1 && profileNumber <= numel(customerNames)
            ...
            warning('Nem talaltam %s nevu oszlopot. A(z) %d. fogyasztoi oszlopot valasztom.', ...
                candidateName, profileNumber);
            return;
        end

        error('Nem talalhato %s, es oszlopindexkent sem ervenyes: %d', ...
            candidateName, profileNumber);
    end

    if ischar(profileNumber) || isstring(profileNumber)

        profileText = upper(strtrim(string(profileNumber)));
        profileText = erase(profileText, " ");

        selectedIdx = find(upper(customerNames) == profileText, 1, 'first');

        if ~isempty(selectedIdx)
            return;
        end

        tok = regexp(char(profileText), 'MT_?(\d+)', 'tokens', 'once');

        if ~isempty(tok)
            n = str2double(tok{1});
            candidateName = string(sprintf('MT_%03d', n));
        ...
        error('Nem talalhato ilyen fogyaszto azonosito: %s', profileText);
    end
...
function [dayMatrix_kW, dayDates, dailyEnergy_kWh, dailyPeak_kW] = ...
    ...
    datesOnly = dateshift(timeVec, 'start', 'day');
    uniqueDays = unique(datesOnly);
...
function fig = local_plot_detailed_profile_figure( ...
    dayMatrix_kW, ...
    dayDates, ...
    dt_h, ...
    selectedCustomer, ...
    ...
    samplesPerDay = size(dayMatrix_kW, 2);
    time_h = (0:samplesPerDay-1) * dt_h;
    ...
    yMax1 = local_max_finite(annualAverageProfile_kW) * 1.15;
    local_draw_window_patch(pvPeakStartHour, pvPeakEndHour, yMax1);

    plot(time_h, annualAverageProfile_kW, ...
        'k', ...
        'LineWidth', 2.5, ...
    ...
    yMax2 = local_max_finite(monthlyAverageProfiles_kW) * 1.15;
    local_draw_window_patch(pvPeakStartHour, pvPeakEndHour, yMax2);
    ...
    for m = 1:12
        if any(isfinite(monthlyAverageProfiles_kW(m, :)))
            plot(time_h, monthlyAverageProfiles_kW(m, :), ...
                'LineWidth', 1.2, ...
    ...
    yMax3 = local_max_finite(dayMatrix_kW) * 1.15;
    local_draw_window_patch(pvPeakStartHour, pvPeakEndHour, yMax3);

    plot(time_h, dayMatrix_kW.', ...
    ...
    imagesc(time_h, 1:size(dayMatrix_kW, 1), dayMatrix_kW);
    set(gca, 'YDir', 'normal');
    grid on;
    box on;
    ...
    hold on;
    xline(pvPeakStartHour, 'w--', 'LineWidth', 1.5);
    xline(pvPeakEndHour, 'w--', 'LineWidth', 1.5);
    hold off;

    saveas(fig, fullfile(savePath, 'selected_load_profile_detailed.png'));
    savefig(fig, fullfile(savePath, 'selected_load_profile_detailed.fig'));
end


function local_draw_window_patch(windowStartHour, windowEndHour, yMax)
    ...
    patch( ...
        [windowStartHour, windowEndHour, windowEndHour, windowStartHour], ...
        [0, 0, yMax, yMax], ...
        [0.90, 0.90, 0.90], ...
        'EdgeColor', 'none', ...
        'FaceAlpha', 0.35, ...
        'DisplayName', sprintf('%.0f-%.0f vizsgalt idoszak', ...
